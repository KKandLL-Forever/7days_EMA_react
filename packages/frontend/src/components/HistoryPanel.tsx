...
export default function HistoryPanel() {
  const { buys, sells, deleteBuyById, deleteSellById } = useAppStore();
  ...
  const allTrades = [
    ...buys.map(b => ({ ...b, _type: 'buy' as const })),
    ...sells.map(s => ({ ...s, _type: 'sell' as const })),
  ].sort((a, b) => b.date.localeCompare(a.date) || (a._type === 'buy' ? -1 : 1));

  const handleDelete = (type: 'buy' | 'sell', id: number) => {
    if (!confirm('删除此条操作记录？')) return;
    type === 'buy' ? deleteBuyById(id) : deleteSellById(id);
  };
          ...
          return (
            <div key={i} className="px-4 py-3 border-b border-gray-100 last:border-b-0 hover:bg-gray-100">
              <div className="flex items-center justify-between mb-1">
                ...
                <button onClick={() => handleDelete(t._type, t.id)}
